...
export const OrderCard: React.FC<OrderCardProps> = ({ order, onStatusChange }) => {
    ...
    // --- MANEJO DE CANCELACIÓN SIMPLE ---
    const handleCancel = () => {
        // Confirmación simple nativa
        if (window.confirm('¿Seguro que deseas CANCELAR esta orden? 🗑️')) {
            onStatusChange(order.orderId, 'cancelled');
            toast('Orden Cancelada', { icon: '🗑️' });
        }
    };
    ...
    return (
        <div 
            className={`
                flex flex-col 
                h-[400px] md:h-[320px] lg:h-[350px] 
                rounded-xl shadow-md overflow-hidden border-l-[6px] ${cardStyle}
                transition-all duration-300 hover:-translate-y-1 hover:shadow-xl animate-fade-in-up relative
                ${isLate && !isReady ? 'ring-2 ring-red-400 animate-pulse' : ''}
            `}
        >
            {/* ICONO DE BASURA SIEMPRE VISIBLE Y FUNCIONAL */}
            {!isReady && (
                <button 
                    onClick={(e) => {
                    ...
                    className="absolute top-1 right-1 p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors z-50 cursor-pointer"
            ...
            <div className="p-2 flex justify-between items-start border-b border-gray-100 shrink-0 bg-white/40 pr-10"> {/* pr-10 para dejar espacio a la basura */}
